...
myproxy = "119.10.67.144:808"

def use_proxy_get_html_data(proxy_addr, url,header):
    try:
        print(proxy_addr)
...
def get_all_hotSong():  # 获取热歌榜所有歌曲名称和id
    url = 'http://music.163.com/discover/toplist?id=724147711'  # 网易云云音乐热歌榜url
    header = {  # 请求头部
        'User-Agent': 'Mozilla/5.0 (X11; Fedora; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36'
    }
    #request = urllib.request.Request(url=url, headers=header)
    #html = urllib.request.urlopen(request).read().decode('utf8')  # 打开url
    html=use_proxy_get_html_data(myproxy,url,header)
...
def get_hotComments(hot_song_name, hot_song_id):
    ...
    '''
    proxy = urllib.request.ProxyHandler({'https': myproxy})
    opener = urllib.request.build_opener(proxy, urllib.request.HTTPHandler)
    urllib.request.install_opener(opener)
    '''
