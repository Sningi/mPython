...
import sys
import random
import argparse  # argparse库用于命令行解析
...
def covertImageToAscii(fileName, cols, scale, moreLevels):
    ...
    for j in range(rows):
        ...
        for i in range(cols):
            ...
            if moreLevels:
                # 平均亮度值[0,255]对应到十级灰度梯度[0,69]，获得对应ASCII符号
                gsval = gscale1[int((avg * 69) / 255)]
            else:
                # 平均亮度值[0,255]对应到七十级灰度梯度[0,9]，获得对应ASCII符号
...
def main():
    ...
    parser.add_argument('--morelevels', dest='moreLevels',
                        action='store_true')  # 设置morelevels为True
    ...
    cols = 80  # 默认分割的列数，列数越大精细度越大，但不建议过大
    if args.cols:
        cols = int(args.cols)
