...
def process_reg_request(packet,gnb_ip,amf_ip):
    # add a new entry and use ran_ue_ngap_id as key in dictionary.
    logger.warning("start processing reg request.")
    ran_ue_ngap_id=packet.ngap.ran_ue_ngap_id.raw_value
    ...
    if packet.ngap.nas_5gs_mm_type_id == '1':
        if hasattr(packet.ngap, 'nas_pdu'):
            ...
            if suci[0] =='0':
                ...
                # if suci is encrypted by profile A
                    ...
                    try:
                        ...
                        # new output would be '01'+imsi_prefix+routing_indicator+'00'+home_network_key_id
                        # +decrypted_msin+padding ff
                        private_key_amf = x25519.X25519PrivateKey.from_private_bytes(private_key)  # private_key class
                        public_key_ue = x25519.X25519PublicKey.from_public_bytes(public_key_ue_bytes)  # public_key class
                        shared_secret_key = private_key_amf.exchange(public_key_ue)  # raw binary string.
                        xkdf = X963KDF(
                            algorithm=hashes.SHA256(),
                        ...
                        suci_icb: bytes = xkdf_output[16:32]
                        suci_mac_key: bytes = xkdf_output[32:]
                        ue_dict[ran_ue_ngap_id][gnb_ip]['suci_enc_key'] = suci_enc_key
                        ue_dict[ran_ue_ngap_id][gnb_ip]['suci_icb'] = suci_icb
                        ue_dict[ran_ue_ngap_id][gnb_ip]['suci_mac_key'] = suci_mac_key
                        # get mac tag from first 8 bytes of the HMAC output.
                        computed_mac_tag:str = HMAC.new(suci_mac_key, encrypted_msin, SHA256).hexdigest()[0:16]
                        if computed_mac_tag == mac_tag_from_message:
                            #first 8 bytes of ICB will be nonce input of AES, and last 8 bytes of ICB will be Initial_value input.
                            ...
                            decrypted_suci =decrypted_suci + (106-len(decrypted_suci))*'f'
                            decrypted_suci_bytes = bytes.fromhex(decrypted_suci)
                            buffer = buffer.replace(bytes.fromhex(suci), decrypted_suci_bytes)
                        else:
                            raise Exception('found mac tag mismatched.')
                    except Exception as e:
                        logger.error("failed to decrypt SUCI based on profileA, operation aborted.\n")
            ...
            if bcd_supi:
                ...
                # ue_dict[ran_ue_ngap_id][gnb_ip]['supi'] = supi
            print("supi:",supi)
            print(type(supi))

def dis_reg():
    filtered_file_name = 'NEA11.pcap'
    # real all contents inside filtered_file_name into buffer.
    with open(filtered_file_name, "rb") as file:
        buffer = file.read()

    # start reading packet in file by call tshark process.
    # to be done: need to make sure the option "try to decode EEA0" is enabled before launch tshark process.
    capture = pyshark.FileCapture(filtered_file_name, display_filter='nas-5gs')
    # if the wireshark was not enabled with "try to decode EEA0" option, the output of tshark would not
    # have message type value in some message like securityMode command/complete, need to figure out a
    # way how to enable that option in wireshark automatically before running tshark.
    packet_number = 0
    for packet in capture:
        ...
        if packet.ngap.procedurecode.raw_value == '0f':
            ...
            # if message type is "registration request".
            if packet.ngap.nas_5gs_mm_message_type.raw_value == '41':
                process_reg_request(packet,gnb_ip,amf_ip)
...
if __name__ == "__main__":
    ...
    supi:bytes = "2089300007487".encode('ascii')
    # computing kausf
    # computing kseaf
    tmp ='082aab7e92b74d029f39aebb1443e68e676b1c1328c4919b22afeaa43b21502d'
    kseaf = binascii.a2b_hex(tmp)
    ...
    algorithm_type_dist = b'\x01'   #type_id for nas_encryption_key
    input_string = b'\x69' + algorithm_type_dist + b'\x00\x01' + \
                    bytes.fromhex('0'+'1') + b'\x00\x01'
    # print(len(input_string))
    input_key = kamf
    # cipher_key uses only last 128 bytes of HMAC output, the bytes string would be 32 bytes long
    # so get the last 16 bytes of bytes string only for cipher_key.
    # should add more logic here, add cipher_key only if auth is successful.
    cipher_key = bytes.fromhex(HMAC.new(input_key, input_string, SHA256).hexdigest())[16:32]

    print_hex(cipher_key)
    kkk = '5a cb 1d 64 4c 0d 51 20 4e a5 f1 45 10 10 d8 52'.replace(" ", "")
    cipher_key=binascii.a2b_hex(kkk)
    print("key:")
    print_hex(cipher_key)
    raw = binascii.a2b_hex('ad 9c 44 1f 89 0b 38 c4 57 a4 9d 42 14 07 e8'.replace(" ",""))
    print("input:")
    print_hex(raw)

    bearer = 0x03
    direction = 0x1       #up: 0 down: 1
    # first_byte_of_bearer_and_direction = (bearer<<3)|(direction<<2)
    count_for_ciphering = 0xFA556B26
    # counter_block = count_for_ciphering.to_bytes(4,byteorder='big') + \
    #                 first_byte_of_bearer_and_direction.to_bytes(1,byteorder='big') + \
    #                 b'\x00\x00\x00' + b'\x00'*8
    # ctr_iv = int(binascii.hexlify(counter_block), 16)
    # ctr_encrypt_counter = Counter.new(128, initial_value=ctr_iv)
    # crypto = AES.new(cipher_key, mode=AES.MODE_CTR, counter =ctr_encrypt_counter)# nonce=counter_block[0:8],initial_value=counter_block[8:16])
    # plain_payload = crypto.decrypt(raw)
 
    # # NEA1
    plain_payload = pysnow.snow_f8(cipher_key, count_for_ciphering, bearer,
                                               direction, raw, len(raw)*8)
    print("PLAN:")
    print_hex(plain_payload)
    # # plain_payload = pyzuc.zuc_eea3(cipher_key, 0, 1,
    # #                                 0, len(raw) * 8, raw) #5gc not support
    '''
    from scapy.all import *

    # newbuff = a[0][SCTPChunkData].data[:31] + plain_payload+ binascii.a2b_hex('0079400f4002f839000000001002f839000001')
    # a[0][SCTPChunkData].data = newbuff
    # wrpcap("/home/song/5G_ciphered_NAS_decipher_tool/src/b.pcap",a)

    a = rdpcap("/home/song/5G_ciphered_NAS_decipher_tool/src/uplink2.pcap")
    newbuff = a[0][SCTPChunkData].data[:31] + plain_payload+ a[0][SCTPChunkData].data[34:]
    a[0][SCTPChunkData].data = newbuff
    
    temp = 'e1 e1 eb 8f 5f fc 4f eb 8b 02 a4 a1 ea 17 b0 8f cb ca c4 72 56 ec e2 77 c9 4c 05 4b 25 f9 c3 96'
    raw2 = binascii.a2b_hex(temp.replace(' ',''))
    count_for_ciphering = 2
    counter_block2 = count_for_ciphering.to_bytes(4,byteorder='big') + \
                    first_byte_of_bearer_and_direction.to_bytes(1,byteorder='big') + \
                    b'\x00\x00\x00' + b'\x00'*8
    ctr_iv = int(binascii.hexlify(counter_block2), 16)
    ctr_encrypt_counter = Counter.new(128, initial_value=ctr_iv)
    crypto = AES.new(cipher_key, mode=AES.MODE_CTR, counter =ctr_encrypt_counter)
    plain_payload2 = crypto.decrypt(raw2)
    print_hex(raw2)
    print_hex(plain_payload2)
    newbuff2= a[0][SCTPChunkData][1].data[:32] + plain_payload2+ a[0][SCTPChunkData][1].data[64:]
    a[0][SCTPChunkData][1].data = newbuff2


    wrpcap("/home/song/5G_ciphered_NAS_decipher_tool/src/uplink2_decrypt.pcap",a)
